...
// Route to update an existing work time entry (departure time) using the PUT method
router.put('/worktimes/departure', async (req, res) => {
  try {
    const { name, departure } = req.body;

    // Find all work times for a given name (allowing multiple entries with the same name)
    const workTimes = await WorkTime.find({ name });

    if (workTimes.length === 0) {
      return res.status(404).json({ error: 'Work time not found' });
    }

    // You might want to update all matching documents if there are multiple entries with the same name
    for (const workTime of workTimes) {
      workTime.departure = departure;
      await workTime.save();
    }

    res.json({ message: 'Departure times updated' });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: 'Internal server error' });
  }
